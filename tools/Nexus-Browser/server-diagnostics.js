...
function createDiagnosticsSnapshot(readContext) {
  return function diagnosticsSnapshot() {
    const { dexStateStore, durability, localTargets, extensionSocket, extensionSessions, uiSockets, lastTabs, lastLocalTargets, dexScheduler, providerControlRouting, providerTargetSpawnRouting, postIdleMaintenance, SERVER_SESSION_ID, ASSET_REVISION, WebSocket } = readContext();
  ...
  return {
    ...
    dexRooms: rooms.length, postIdle: postIdleMaintenance?.diagnostics?.() || null, recoveryRooms: rooms.filter((room) => !!room.recovery).length, savedAt: snapshot?.savedAt || null,
