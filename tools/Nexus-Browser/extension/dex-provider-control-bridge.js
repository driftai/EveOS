(() => {
  ...
  const boundWatchApi = globalThis.BrowserAiBridgeDexBoundTabWatchdog
  ...
  const boundTabWatchdog = boundWatchApi?.createBoundTabWatchdog({ getSocket: () => socket });
  ...
  function handleServerMessage(raw) {
    ...
    if (boundTabWatchdog?.handleServerMessage(msg)) return;
    ...
    connecting = (async () => {
      ...
      return new Promise((resolve, reject) => {
        ...
        ws.addEventListener('open', () => {
          ...
          setTimeout(() => boundTabWatchdog?.request(), 50);
          resolve(ws);
        }, { once: true });
        ws.addEventListener('error', () => {
          clearTimeout(timer);
