## Async Astro completion, NOTE and FIFO recovery

The reported sequence had Astro's local CLI still displaying an active `1 task(s) · /tasks` indicator while an intermediate relay note was captured. The existing terminal parser could mistake a visible empty input prompt for a finished result even though tests were still executing; the updated parser requires the final prompt to have **no nonzero task counter after it** before it emits `response_final`. Dedicated regression tests in `tests/antigravity-active-task-finalization.test.js` exercise both parser readiness and the Existing Session send observer. A genuine final report remains tied to its original request ID and durable journal; no replay.

If Eve sends NOTE or an agent finishes later, an authenticated new `send` now persists immediately to an exact-member, exact-room, request-ID-deduplicated FIFO even when a relay/recovery or Dex UI is busy. A new report does not interrupt the old gesture. At the next committed/reconciled boundary, Dex batches at most eight new message IDs into the next envelope, FIFO; overflow is explicit and does not append a phantom transcript entry. An idle NOTE-stopped room begins a fresh, separately identified relay after existing recovery and pending control receipts are reconciled. Existing queued data and in-flight source IDs remain server authoritative against stale browser snapshots. Direct sends must not leave an extra pending control receipt after the originating model turn later finalizes; private `room_log`/`room_budget` lookups are never held for the old reply's four-minute correlation timeout.

A truly detached local task still requires the already-supported signed `task-completion-runner.js start` registration to send its result later without the relay. A synthetic terminal transcript alone cannot prove a disconnected job was registered or still being observed. No uncertain old dispatch is replayed; late archived finals remain inert and explicit room messages enter the FIFO under their own request IDs. Native Node, Windows existing-session terminal and headed browser qualification on the exact new SHA remain required before deployment or main promotion.

## New Dex room budget/context verification

The relay payload now carries a physical turn ledger (allocated/scheduled/unscheduled) and defaults to one last earlier message PER room agent. New trailing flags `[[DEX:CONTEXT:12]]` and `[[DEX:BUDGET:+4]]` allow temporary context expansion and budget extension without stopping the relay; the **last scheduled agent** gets a one-time, capacity-aware `FINAL-TURN BUDGET NUDGE` within its existing prompt and may choose to add `[[DEX:BUDGET:+N]]` BEFORE its RETURN marker; without that choice the room stops for budget, not DONE; the authoritative current source message remains full. New localhost-owned commands: `room_budget`, idle `set_room_budget` (optional `resume:true`) and read-only `room_log` (cursor pagination, requester's exact room only). `room_log` never appends transcript messages. Explicit CMD during an in-flight reply still pauses that relay, so read/change between turns when possible. The old `contextMessages:8` setting remains loadable but no longer causes every turn to carry eight chronological messages; explicit new `contextDefaultMessages` can opt a room into a larger permanent window. An interrupted mailbox send can carry `contextMessages:N` for its one queued source. Preserve old messages and receipt ledgers; never auto-replay uncertain sends. Headed live proof remains required.
