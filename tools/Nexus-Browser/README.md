...
### Dex control waits for origin finalization (server-side repair)

The originating ChatGPT reply can expose a trailing Dex CMD while it is still thinking or while the final-response outbox is merely queued. The older eight-second server correlation window could reject that command before the relay turn was durably committed, so Astro never received it. Provider controls now wait up to four minutes for an **exact, durably committed** originating turn and matching command before execution, while immediately rejecting an ambiguous, changed, expired or unrelated turn. An early `provider_control_received` acknowledges transport only; it does not prove execution or recipient delivery. This is a server-only repair: deploy through supervised restart after the global idle and draft-safety gate; no extension revision bump or blind command replay. Qualify with `node --test tests/dex-origin-finalization.test.js`, full Nexus gates, and live single-send proof; report any timeout with the actual originating request ID.

### ChatGPT article-turn and exact-composer submission recovery (adapter revision 45)

The headed revision-44 deployment passed local tests but still exposed zero assistant nodes on the exact bound tab. Revision 45 adds a fallback for role-free article turns containing `.markdown.prose`, with explicit user veto and nested-article checks. Dex prompt submission now scopes the Send control to the exact composer, refuses to overwrite unrelated drafts and attempts just one real Send click (or native form submission/Enter only if no Send button is available). An unconfirmed submission leaves the draft intact and reports failure instead of replaying uncertain actions. This is remote source/simulation evidence pending Astro's local and live qualification; do not promote the branch before a real status-command round-trip.

### ChatGPT semantic turn recovery (adapter revision 44)

A live revision-43 proof on September 25, 2026 found a new ChatGPT renderer variant where the exact bound tab was healthy and the Dex watcher was running, but the answer extractor reported zero assistant nodes. Revision 44 keeps the guarded DIL fallback and adds semantic `data-turn="assistant|user"` ownership so `section[data-turn]` / equivalent turn shells are recognized without treating a bare selection id as assistant-owned. This preserves the user/assistant safety boundary while restoring trailing Dex command discovery on the newer renderer.

### Passive Dex-bound ChatGPT self-heal (adapter revision 43)

After revision 43 is installed once, the provider-control worker asks localhost every five seconds for the **exact ChatGPT tabs that are members of durable Dex rooms**. A healthy current tab receives a cheap rescan so a visible trailing Dex marker cannot remain unnoticed. If only the Dex watcher is missing, Nexus injects that watcher without navigating the page. If the tab is on a stale adapter revision, Nexus may reload only that exact bound tab **after all durable rooms, recoveries, provider controls and scheduler work are idle** and only when no unsent ChatGPT composer draft is detected. The stale-tab reload is claimed once per expected revision to prevent loops.

The same snapshot advertises the server's expected adapter revision. Once revision 43 is already running, a future stale extension service worker can self-reload once when global Dex state is safe, then repair/reload exact bound tabs as needed. This cannot bootstrap an extension version that predates the watchdog itself: the first revision-43 deployment still needs the supervised extension refresh/site refresh procedure. After that bootstrap, routine stale-worker/stale-tab gaps self-heal instead of depending on a human page reload.

### Valid Dex command ownership handshake (adapter revision 42)

A visible, valid trailing `[[DEX:CMD ...]]` is no longer treated as dispatched merely because ChatGPT handed it to Chrome. The MV3 service worker keeps its reply channel open and localhost emits `provider_control_received` after validating the exact source. Only that ownership receipt enters duplicate suppression. A definite pre-dispatch failure can retry from the same visible marker; an uncertain post-send outcome remains fail-closed and is never automatically replayed. This closes the valid-marker black-hole gap between the content watcher, extension worker and localhost.
