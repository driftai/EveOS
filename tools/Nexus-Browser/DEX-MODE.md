...
By default the next recipient gets its source message **unabridged** plus exactly the latest earlier message from each room agent (one from Eve and one from Astro in a two-agent room); prior human/system messages are not automatically bundled. Historical transcripts remain intact. Context selection is source-ordered, so a delayed queued message cannot see newer room messages. Per-hop `[[DEX:CONTEXT:12]]` requests up to 12 prior messages in the NEXT recipient's payload only (1–40, retains member anchors, max 24k projected characters). Out-of-band `[[DEX:CMD {"action":"send","text":"...","contextMessages":12,"relay":true}]]` also allows one-hop expansion.

Every agent can inspect its authorized room without writing a room message: `[[DEX:CMD {"action":"room_log","room":"<exact room ID>","limit":10}]]`. The server returns a PRIVATE, backward-pageable log to that requester. Use `before` with `nextBefore` from the previous page; limits are 1–25, or at most 5 when `full:true`. Logs remain available when the Dex viewer is offline; out-of-band read-only calls do not add room messages, subscribe to notifications, or start a relay. A trailing CMD issued **during an active agent reply stops that relay by existing design** and may record a metadata-only control receipt; room_log text stays private: use out-of-band commands when idle or explicitly resume afterward, not an automatic hidden re-run.
