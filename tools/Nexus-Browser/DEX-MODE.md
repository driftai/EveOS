## Live room budget and context tools (September 2026)

Dex now distinguishes **turn budget exhaustion** from agent-declared completion: `Relay budget complete` means the configured turn count was scheduled, NOT that any agent sent DONE. Every relay envelope reports allocated, scheduled and unscheduled turns; the currently dispatched turn is already counted. The UI displays the same ledger.

By default the next recipient gets its source message **unabridged** plus exactly the latest earlier message from each room agent (one from Eve and one from Astro in a two-agent room); prior human/system messages are not automatically bundled. Historical transcripts remain intact. Context selection is source-ordered. Newly admitted, authenticated inbound reports are batched in FIFO order at the next safe hop, separately from the default one-message-per-agent backlog; a delayed queued source never silently sees newer unrelated room messages. Per-hop `[[DEX:CONTEXT:12]]` requests up to 12 prior messages in the NEXT recipient's payload only (1–40, retains member anchors, max 24k projected characters). Out-of-band `[[DEX:CMD {"action":"send","text":"...","contextMessages":12,"relay":true}]]` also allows one-hop expansion.

Every agent can inspect its authorized room without writing a room message: `[[DEX:CMD {"action":"room_log","room":"<exact room ID>","limit":10}]]`. The server returns a PRIVATE, backward-pageable log to that requester. Use `before` with `nextBefore` from the previous page; limits are 1–25, or at most 5 when `full:true`. Logs remain available when the Dex viewer is offline. Authenticated read-only `room_log` and `room_budget` skip the four-minute originating-turn settle wait, disclose data only to the requesting member, and do not add room messages, subscribe to notifications, or start a relay. A trailing CMD issued **during an active agent reply stops that relay by existing design** and may record a metadata-only control receipt; room_log text stays private: use out-of-band commands when idle or explicitly resume afterward, not an automatic hidden re-run.

For budget inspection, use `[[DEX:CMD {"action":"room_budget"}]]`. To set the **next** room budget while safely idle: `[[DEX:CMD {"action":"set_room_budget","turns":12,"resume":true}]]`; `resume:true` schedules exactly one new relay from the last committed room message after recording settings, not a replay of an uncertain turn. Busy or recovering rooms reject replacement. During your own in-flight reply, `[[DEX:BUDGET:+4]]` extends the existing room by four *unscheduled* turns before the zero-budget stop decision, capped at 500 allocated turns per run; it does not require a CMD pause. The **final scheduled recipient** also gets one prominent, capacity-aware `FINAL-TURN BUDGET NUDGE` in its existing relay envelope (not a new message or extra turn): it explicitly asks the agent to decide whether more work justifies `[[DEX:BUDGET:+N]]` in that very reply, BEFORE RETURN/DONE. No flag means ordinary budget stop; DONE/USER/NOTE/CMD always stop instead of extending. The nudge disappears on non-final turns, indicates when the 500-turn hard cap prevents extension, and never changes the configured room default. Invalid/duplicated flags fail closed.

No feature overrides exact source-room authorization, committed-turn receipts, a draft-safe delivery boundary or the recovery mailbox. Source-isolated simulations are useful remote evidence but not headed-browser qualification. Astro must run local focused/full tests, root smoke, guardrails and AI-control on the exact branch SHA and then prove a nonduplicating live exchange before promotion to `main`.

## Interrupted-room deadlock prevention: durable mailbox + recovery deadline

A new authenticated `send` with `relay:true` is admitted directly to localhost's durable FIFO (eight-message backpressure) while a different agent is running, a room is NOTE-stopped, a recovery is pending, or the Dex viewer is disconnected. An exact, stable request ID prevents duplicates across reconnects; a conflicting payload, ambiguous room/binding or unauthorized sender fails closed. The queued receipt proves persistence, NOT provider delivery. The scheduler stages the FIFO only after the old request's safe commit or passive archival; the next prompt carries queued message IDs and their projected bodies in order. The room's `status` exposes `deferredSends` and `recoveryPassiveAt`. A recovering prompt is NEVER replayed just because another report arrived.
...
keeping the Eve–Astro Dex relay open. **If Astro may still be running tests when Eve sends NOTE, register this exact branch/SHA background job BEFORE ending the relay**. An arbitrary disconnected terminal task is not automatically observed or retried; without a registered job, Astro must explicitly `send` its completed report into the durable inbox:
...
`rooms`, `use_room`, and `status` return a compact tool result to the originating browser provider so it can navigate the rooms it is already authorized to use. A successful `send` is silent by default and records the initiating provider as the room speaker before optionally starting the normal round-robin relay. Authenticated `send` with `relay:true` instead commits an independent source message into the server's durable FIFO while the room is busy, without touching an in-flight provider gesture. The result reports `queued`, `batched` or `dispatched`, never falsely `delivered`. A direct SEND has its own request-ID receipt and MUST NOT create a second post-final pending-control receipt that blocks queue drainage.
...
9. `[[DEX:NOTE]]` records the sender's message and stops normal round-robin relay; it does not mark a task complete. Independent, authenticated new reports already admitted to the durable FIFO may start a NEW relay after this turn safely settles.
