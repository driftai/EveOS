...
async function saveIdentifiers(song, identifiers) {
...
function filteredSongs(songs, prefs) {
  const filtered = songs.filter(song => {
    ...
    const rating = Number(custom.personal_rating);
    const conversion = Number(auto.conversion_rating);
    const duration = Number(auto.duration_ms || song.duration_ms || 0) / 60000;
    const events = Number(auto.event_count || 0);
...
async function installPlanner() {
  ...
  function renderFilterOptions() {
    const genres = state.songs.flatMap(song => listValue(song.identifiers?.genre));
    const authors = state.songs.map(song => song.identifiers?.author || song.artist).filter(Boolean);
    const hosts = state.songs.map(song => song.automatic_identifiers?.source_host).filter(Boolean);
    const kinds = state.songs.map(song => song.automatic_identifiers?.kind || song.kind).filter(Boolean);
    optionList(els.genre, genres, "All genres"); optionList(els.author, authors, "All authors"); optionList(els.host, hosts, "All sources"); optionList(els.kind, kinds, "All types");
  }
  ...
  function openEditor(song) {
    state.editingId = song.id; const ids = song.identifiers || {}; const custom = ids.custom || {};
  ...
  function renderResults() {
    ...
    els.count.textContent = `${visible.length} matches`;
    els.selection.textContent = `${state.selected.size} selected`;
  ...
  async function refresh() {
    try { state.songs = await fetchSongs(); renderFilterOptions(); renderResults(); } catch (error) { toast(error.message, "error"); }
  }
  ...
  async function saveEditor() {
    ...
    try {
      ...
      const updated = await saveIdentifiers(song, identifiers); state.songs = state.songs.map(item => String(item.id) === String(updated.id) ? updated : item); renderFilterOptions(); renderResults(); openEditor(updated); toast("Identifiers saved to the Local Library", "complete");
    } catch (error) { toast(error.message, "error"); }
  }
...
if (document.readyState === "loading") {
  window.addEventListener("DOMContentLoaded", () => { void installPlanner(); }, { once: true });
} else {
  void installPlanner();
}
